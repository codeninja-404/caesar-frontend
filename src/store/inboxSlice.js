...
export const deleteEmail = createAsyncThunk('inbox/deleteEmail', async ({ inboxId, emailId }) => {
    const response = await fetch(`http://localhost:8080/inboxes/${inboxId}/emails/${emailId}`, {
        method: 'DELETE',
    });
    if (!response.ok) throw new Error(`Failed to delete email ${emailId} from inbox ${inboxId}`);
    return { inboxId, emailId };
});
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(deleteEmail.fulfilled, (state, action) => {
                state.status = 'succeeded';
                const { inboxId, emailId } = action.payload;
                state.emails[inboxId] = state.emails[inboxId].filter((email) => email.id !== emailId);
            })
            .addCase(deleteEmail.rejected, (state, action) => {
                state.status = 'failed';
                state.error = action.error.message;
                console.error('Delete email failed:', action.error.message);
            });
    },
});
